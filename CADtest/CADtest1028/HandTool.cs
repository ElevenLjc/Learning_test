...
namespace CAD
{
    public partial class HandTool : BaseTool
    {
        ...
        public override void mouseDrag(object sender, MouseEventArgs e)//重写鼠标的拖动事件
        {
            ...
            {
                Point setPoint = this.getDragPoint();
                if (catchPointIndex == 0)//如果捕捉到移动点时
                {
                    setPoint = new Point();
                    setPoint.X = this.getDragPoint().X - this.getOldDragPoint().X;//计算增量点
                    setPoint.Y = this.getDragPoint().Y - this.getOldDragPoint().Y;//计算增量点
                }
