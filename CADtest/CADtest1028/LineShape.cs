...
namespace CAD
{
    ...
    {
        public static bool IsInLine(Point p1, Point p2, Point mp)//判断鼠标位置mp是否在线段p1、p2上（0.1范围内），如果是返回真，否则返回假
        {
            double iLen1 = Math.Pow(p1.X - p2.X, 2) + Math.Pow(p1.Y - p2.Y, 2);
            double iLen2 = Math.Pow(p1.X - mp.X, 2) + Math.Pow(p1.Y - mp.Y, 2);
            double iLen3 = Math.Pow(p2.X - mp.X, 2) + Math.Pow(p2.Y - mp.Y, 2);
